...
oneof str = P
  (\case
    Nil       -> UnexpectedEof
    (x :. xs) -> if x `elem` str then Result xs x else UnexpectedChar x
  )
...
noneof str = P
  (\case
    Nil       -> UnexpectedEof
    (x :. xs) -> if x `notElem` str then Result xs x else UnexpectedChar x
  )
...
between left right middle = (\_ -> const middle' =<< right') =<< left
 where
  right' = P
    (\i -> case parse right (reverse i) of
      Result i' a' -> Result (reverse i') a'
      anything     -> anything
    )
  middle' = (\a -> P (\i -> if isEmpty i then Result i a else UnexpectedString i)) =<< middle
...
-- True
hex :: Parser Char
hex = toHexChar =<< thisMany 4 (satisfy isHexDigit)
 where
  toHexChar a = P
    (\i -> case readHex a of
      Empty  -> UnexpectedString a
      Full h -> Result i (chr h)
    )
...
sepby1 pa ps = (\a -> (a :.) <$> list (const pa =<< ps)) =<< pa
...
sepby pa ps = sepby1 pa ps ||| Nil <$ eof
...
satisfyAll ls = P
  (\case
    Nil       -> UnexpectedEof
    (x :. xs) -> if all (\f -> f x) ls then Result xs x else UnexpectedChar x
  )
...
satisfyAny ls = P
  (\case
    Nil       -> UnexpectedEof
    (x :. xs) -> if any (\f -> f x) ls then Result xs x else UnexpectedChar x
  )
