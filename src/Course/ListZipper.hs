...
findLeft f (ListZipper (l :. ls) a r) =
  let next = ListZipper ls l (a :. r) in if f l then IsZ next else findLeft f next
...
findRight f (ListZipper l a (r :. rs)) =
  let next = ListZipper (a :. l) r rs in if f r then IsZ next else findRight f next
...
moveLeftLoop (ListZipper Nil       a r) = let (x :. xs) = reverse (a :. r) in ListZipper xs x Nil
...
moveRightLoop (ListZipper l a Nil      ) = let (x :. xs) = reverse (a :. l) in ListZipper Nil x xs
...
moveLeftN' n lz@(ListZipper l _ _) | n < 0     = moveRightN' (-n) lz
                                   | n == 0    = Right lz
                                   | otherwise = foo n lz
 where
  foo n' lz'@(ListZipper l' _ _)
    | n' == 0 = Right lz'
    | otherwise = case l' of
      Nil -> (Left . length) l
      _   -> foo (n' - 1) (moveLeftLoop lz')
...
moveRightN' n lz@(ListZipper _ _ r) | n < 0     = moveLeftN' (-n) lz
                                    | n == 0    = Right lz
                                    | otherwise = foo n lz
 where
  foo n' lz'@(ListZipper _ _ r')
    | n' == 0 = Right lz'
    | otherwise = case r' of
      Nil -> (Left . length) r
      _   -> foo (n' - 1) (moveRightLoop lz')
...
nth n lz@(ListZipper l _ r) | n < 0           = IsNotZ
                            | n < lenl        = toMaybeLZ $ moveLeftN' (lenl - n) lz
                            | n == lenl       = IsZ lz
                            | n < lenl + lenr = toMaybeLZ $ moveRightN' (lenl + 1 + lenr - n) lz
                            | otherwise       = IsNotZ
 where
  lenl = length l
  lenr = length r
  toMaybeLZ (Left  _  ) = IsNotZ
  toMaybeLZ (Right lz') = IsZ lz'
...
end (ListZipper l a (r :. rs)) =
  let (x :. xs) = reverse (r :. rs) in ListZipper (xs ++ a :. l) x Nil
...
start (ListZipper (l :. ls) a r) =
  let (x :. xs) = reverse (l :. ls) in ListZipper Nil x (xs ++ a :. r)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper fl fa fr) (ListZipper l a r) =
    ListZipper ((\(f, a') -> f a') <$> zip fl l) (fa a) ((\(f, a') -> f a') <$> zip fr r)
...
instance Extend ListZipper where
  (<<=) f lz = ListZipper (allLeft (moveLeftN' 1 lz)) (f lz) (allRight (moveRightN' 1 lz))
   where
    allLeft (Left  _  ) = Nil
    allLeft (Right lz') = f lz' :. allLeft (moveLeftN' 1 lz')
    allRight (Left  _  ) = Nil
    allRight (Right lz') = f lz' :. allRight (moveRightN' 1 lz')
...
instance Extend MaybeListZipper where
  (<<=) _ IsNotZ   = IsNotZ
  (<<=) f (IsZ lz) = IsZ (f' <<= lz) where f' = f . IsZ
