...
(|||) (P f1) (P f2) = P (\i -> let r1 = f1 i in if isErrorResult r1 then f2 i else r1)
...
satisfy f = (\c -> if f c then pure c else unexpectedCharParser c) =<< character
...
sequenceParser = (reverse <$>) <$> foldLeft (\pla pa -> (\l -> (:. l) <$> pa) =<< pla) (pure Nil)
...
firstNameParser = lift2 (++) (thisMany 1 upper) (list lower)
...
surnameParser = lift3 (\a b c -> a ++ b ++ c) (thisMany 1 upper) (thisMany 5 lower) (list lower)
...
phoneParser =
  lift3 (\a b c -> a ++ b ++ c) (thisMany 1 digit) phoneBodyParser (thisMany 1 (is '#'))
...
personParser =
  foo
    ...
    <*> phoneParser
 where
  foo age _ firstName _ surname _ isSmoker _ phone = Person age firstName surname isSmoker phone
